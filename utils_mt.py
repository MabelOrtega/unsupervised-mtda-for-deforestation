...
def retrieve_num_samples_base(current_folder, list_dir, folder):
    content_list = {}
    for index, val in enumerate(list_dir):
        path = os.path.join(current_folder, val, 'patches', folder, 'path_image')
        #print(path)
...
def retrieve_num_samples(current_folder, list_dir, folder, patches_tg):
    content_list = {}
...
def data_gen_base(current_folder, list_dir, folder, batch_size, patch_size, channels, num_classes):
    
    complete_list, _ = retrieve_num_samples_base(current_folder, list_dir, folder)
    c = 0
    ...
    while (True):
        ...
        for i in range(c, c+batch_size): #initially from 0 to 16, c = 0. 
            ...
            img_folder  = current_folder + name_data + '_1C/' + 'patches/' + folder + '/path_image/'
            mask_folder = current_folder + name_data + '_1C/' + 'patches/' + folder + '/path_mask/'
...
def data_gen(dir_data, list_dir, folder, batch_size, patch_size, channels, num_classes, patches_tg):
    c = 0
